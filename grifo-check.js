...
var saldo_current = 0;
var idx_hoje = 0;
var id_cookie = 'grifo_saldo-'+$('input[name=data]').val();
...
var setCkInputValues = function() {
  ...
  if($('.minhaJornada').length && $('.meuPonto').length) {
    ...
    //APAGA POSICOES VAZIAS


    //ARMAZENA COOKIE
    setDadosCookie({'arr_jornadas_ck':arr_jornadas_input,'arr_horarios_ck':arr_horarios_input});  
  }
...
var getArrResultCk = function(/*arr_jornadas_grv,arr_horarios_grv*/) {
  ...
  var arr_cookie = Object.keys(getDadosCookie()).length ? getDadosCookie() : {};  
  ...
  for(var i=0;i<total_dias;i++) {
    ...
    var arr_grv = arr_horarios_ck[i] ? arr_horarios_ck[i] : (arr_horarios_orig[i] ? arr_horarios_orig[i] : []);
    ...
    if(arr_grv.length) {
      ...
      for(var batida in arr_grv) {
        ...
        var b1 = arr_horarios_ck[i] && arr_horarios_ck[i][batida] && arr_horarios_ck[i][batida][0] ? arr_horarios_ck[i][batida][0] : (arr_horarios_orig[i][batida] ? arr_horarios_orig[i][batida][0] : []);
        var b2 = arr_horarios_ck[i] && arr_horarios_ck[i][batida] && arr_horarios_ck[i][batida][1] ? arr_horarios_ck[i][batida][1] : (arr_horarios_orig[i][batida] ? arr_horarios_orig[i][batida][1] : []);

        arr_horarios_result[i][batida][0] = b1;
        arr_horarios_result[i][batida][1] = b2;
  ...
  for(var dia in arr_jornadas) {
    ...
    if(cnt_hoje) {
      for(var batida in arr_horarios[dia]) {      
        ...
        var b_hj_chk = b_hj_chk && pattern_h.test(b1) && pattern_h.test(b2) ? true : false;
      }
    }
    //CALCULO DE JORNADA
  ...
  for(var dia in arr_horarios) {
    ...
    for(var batida in arr_horarios[dia]) {
      ...
      if(pattern_h.test(b1) && pattern_h.test(b2)) {
        ...
        saldo_horario_mes += diffDate(b1,b2);
...
var getDadosCookie = function() {

  var arr_cookie = getCookie(id_cookie);  
  ...
  return {'arr_jornadas_ck':arr_jornadas_ck,'arr_horarios_ck':arr_horarios_ck};

}

var setDadosCookie = function(obj) {

  var arr_cookie = [obj['arr_jornadas_ck'],obj['arr_horarios_ck']];  

  return setCookie(arr_cookie);
...
/*var submitForm = function() {
  ...
  var arr_input = setCkInputValues();
...
var check = function() {
  ...
  if($('#divSecaoResult .titulosecao_table').length && !$('#conteinerSaldoTotal').length) {
...
var setCookie = function(arr_cookie) {
  
  try {
    var str_cookie = JSON.stringify(arr_cookie);
    Cookies.remove(id_cookie);
    Cookies.set(id_cookie,str_cookie);
  }
  catch(e) {
    return false;
  }
...
var getCookie = function() {
  ...
  try {
    var arr_cookie = JSON.parse(Cookies.get(id_cookie));
  }
  catch(e) {
    return [];
  }
